var et=Object.defineProperty;var nt=(e,n,t)=>n in e?et(e,n,{enumerable:!0,configurable:!0,writable:!0,value:t}):e[n]=t;var w=(e,n,t)=>nt(e,typeof n!="symbol"?n+"":n,t);import{R as st,P as at}from"./sim-B88PxYSu.js";const z=4,it=String.raw`
...
`,ot=10.2,rt=.004,ht=.28,H=6.1,lt=343,q=1.6/H;function ct(e,n){const t=q*n;return t/Math.hypot(e,t)}const dt=.62,ut=1500,ft=.9,pt=.084,wt=[1,9/8,5/4,3/2,5/3],gt=196;function mt(e){let n=e,t=1/0;for(let s=-1;s<=4;s++){const o=gt*Math.pow(2,s);for(const a of wt){const i=o*a,r=Math.abs(Math.log2(i/e));r<t&&(t=r,n=i)}}return n}function yt(e,n,t){const s=Math.floor(e.sampleRate*n),o=e.createBuffer(2,s,e.sampleRate);for(let a=0;a<2;a++){const i=o.getChannelData(a);for(let h=0;h<s;h++){const c=h/s;i[h]=(Math.random()*2-1)*Math.pow(1-c,t)}const r=Math.floor(e.sampleRate*.012);i.copyWithin(r,0,s-r),i.fill(0,0,r)}return o}class vt{constructor(){w(this,"ctx",null);w(this,"node",null);w(this,"master",null);w(this,"ready",!1);w(this,"opened",!1);w(this,"budget",0);w(this,"binaural",!0)}get running(){return this.ready}get spatial(){return this.binaural}async start(){if(this.ctx){await this.ctx.resume(),this.opened&&this.fadeIn();return}const n=window.AudioContext??window.webkitAudioContext;if(!n)throw new Error("Web Audio is unavailable");const t=new n({latencyHint:"interactive"});this.ctx=t;const s=t.resume();t.addEventListener("statechange",()=>{this.opened&&this.fadeIn()});try{const o=URL.createObjectURL(new Blob([it],{type:"text/javascript"}));try{await t.audioWorklet.addModule(o)}finally{URL.revokeObjectURL(o)}const a=new AudioWorkletNode(t,"bowl-processor",{outputChannelCount:[z]});this.node=a;const i=t.createChannelSplitter(z);a.connect(i);const r=tt=>{const I=t.createBiquadFilter();I.type="highpass",I.frequency.value=90;const E=t.createBiquadFilter();E.type="highshelf",E.frequency.value=3200,E.gain.value=-9;const x=t.createBiquadFilter();return x.type="lowshelf",x.frequency.value=320,x.gain.value=2,tt.connect(I),I.connect(E),E.connect(x),x},h=t.createGain();h.gain.value=.9;const c=t.createGain();c.gain.value=1;const p=t.createConvolver();p.buffer=yt(t,1.3,3.6);const d=t.createChannelMerger(2);i.connect(d,0,0),i.connect(d,1,1),r(d).connect(h);const R=t.createChannelMerger(2);i.connect(R,2,0),i.connect(R,3,1),r(R).connect(p);const y=t.createDynamicsCompressor();y.threshold.value=-4,y.knee.value=3,y.ratio.value=12,y.attack.value=.004,y.release.value=.18;const M=t.createGain();M.gain.value=0,this.master=M,p.connect(c),h.connect(M),c.connect(M),M.connect(y),y.connect(t.destination),await s,this.opened&&this.fadeIn()}catch(o){throw await t.close().catch(()=>{}),this.ctx=null,this.node=null,this.master=null,o}}open(){this.opened=!0,this.fadeIn()}fadeIn(){const n=this.ctx,t=this.master;!n||!t||this.ready||n.state!=="running"||(t.gain.setValueAtTime(0,n.currentTime),t.gain.linearRampToValueAtTime(.85,n.currentTime+2.4),this.ready=!0)}setBinaural(n){var t;this.binaural=n,(t=this.node)==null||t.port.postMessage({type:"binaural",value:n})}beginFrame(){this.budget=10}play(n){if(!this.ready||!this.node||this.budget<=0)return!1;this.budget--;const t=n.strike,s=n.wall,o=mt(ot/Math.max(rt,n.relSize)),a=Math.min(1,n.relDv/ht),i=Math.hypot(n.dx,n.dy),r=Math.atan2(n.dx,-n.dy),h=Math.hypot(i,q*n.radius),c=h/n.radius,p=i/h,d=Math.max(0,-Math.cos(r))*p;return this.node.port.postMessage({type:"hit",f0:o,az:r,spatial:p,flight:c*H/lt,cutoff:Math.max(ut,2e4*Math.exp(-(dt*c+ft*d))),send:pt/n.presence*(1+.25*d),amp:(s?.11:.34)*a*n.presence,decay:(.5+32*n.relSize)*(s?.3:1)*(.85+.3*t.bowl.timbre)*(.7+.45*a),bright:s?-.4:-.12+a*.3,stretch:(s?.02:.004)+t.bowl.timbre*.01,contact:s?.012:.006}),!0}}const A=document.getElementById("stage"),bt=document.getElementById("veil"),Z=document.getElementById("hud"),k=new st(A),u=new vt;function At(e){let n=e>>>0;return()=>{n=n+1831565813>>>0;let t=Math.imul(n^n>>>15,1|n);return t=t+Math.imul(t^t>>>7,61|t)^t,((t^t>>>14)>>>0)/4294967296}}const X=3,Mt=[162,198,144],Et=.8/3,xt=2/3,It=2.6,Ot=.21,N={x:-.644,y:-.271},St=.08;function _t(e,n){return Math.max(e,n)/(2*xt)}let g=[],v=0,b=0;const f={x:0,y:0},m={left:0,top:0,width:0,height:0};function G(e,n){const t=_t(e,n),s=It*t/Math.sqrt(3);return Array.from({length:X},(o,a)=>{const i=Ot+a*2*Math.PI/X;return{cx:Math.cos(i)*s,cy:Math.sin(i)*s,r:t}})}function j(e){const n=e[0].r*(1+St),t=Math.min(...e.map(o=>o.cx-n)),s=Math.min(...e.map(o=>o.cy-n));m.left=t,m.top=s,m.width=Math.max(...e.map(o=>o.cx+n))-t,m.height=Math.max(...e.map(o=>o.cy+n))-s}function V(e,n,t,s){return t<=s?n+t/2:Math.min(Math.max(e,n+s/2),n+t-s/2)}function P(e,n){f.x=V(e,m.left,m.width,v),f.y=V(n,m.top,m.height,b)}function kt(){const e=G(v,b);j(e);const n=At(20932);g=e.map((t,s)=>{const o=1.5+s*.35,a=.085+s*.01,i=a*Et,r=new at(t.cx,t.cy,t.r,{scale:o,drift:i*o,strength:i,jet:a,axis:(s%2===0?1:-1)*(.7+s*.9),width:.34,confine:.05,seed:17.3*(s+1)},n);return r.populate(Mt[s]),r}),P(N.x*e[0].r,N.y*e[0].r),k.invalidate()}function Rt(e){const n=G(v,b),t=n[0].r/g[0].radius,s=e.x*t,o=e.y*t;j(n),g.forEach((a,i)=>{const r=n[i];a.cx=r.cx,a.cy=r.cy,a.radius=r.r;for(const h of a.bowls)h.x*=t,h.y*=t,h.r*=t,h.m*=t*t,h.vx*=t,h.vy*=t;a.ripples.length=0}),P(s,o),k.invalidate()}function Y(){const e=Math.min(window.devicePixelRatio||1,2),n=document.documentElement.clientWidth,t=document.documentElement.clientHeight;if(n<=0||t<=0)return;const s=g.length?{x:f.x,y:f.y}:null;v=n,b=t,A.width=Math.round(v*e),A.height=Math.round(b*e),k.setDpr(e),s===null?kt():Rt(s)}const C=1/120,W=6,O=[];let S=0,B=performance.now(),D=60,L=0,T=0,_=!1;function $(e){requestAnimationFrame($);let n=(e-B)/1e3;B=e,n>.25&&(n=.25),D+=(1/Math.max(n,1e-4)-D)*.05,S+=n;let t=0;for(;S>=C&&t<W;){for(const a of g)a.step(C);S-=C,t++}t===W&&(S=0),u.beginFrame();let s=0,o=0;O.length=0;for(const a of g){s+=a.contacts.length;for(const i of a.contacts){const r=i.x-f.x,h=i.y-f.y,c=ct(Math.hypot(r,h),a.radius),p=i.kind==="wall";for(const d of i.strikes)O.push({strike:d,wall:p,relSize:d.bowl.r/a.radius,relDv:d.dv/a.radius,dx:r,dy:h,radius:a.radius,presence:c})}a.contacts.length=0}O.sort((a,i)=>i.relDv*i.presence-a.relDv*a.presence);for(const a of O)u.play(a)&&o++;if(L+=(s/Math.max(n,1e-4)-L)*.03,T+=(o/Math.max(n,1e-4)-T)*.03,k.frame(v,b,g,f),_){const a=g.reduce((i,r)=>i+r.bowls.length,0);Z.textContent=`${D.toFixed(0)} fps   ${a} bowls   ${L.toFixed(1)} contacts/s   ${T.toFixed(1)} voices/s   audio ${u.running?"on":"off"}   ${u.spatial?"binaural":"stereo"}`}}window.addEventListener("keydown",e=>{e.key==="h"?(_=!_,Z.classList.toggle("on",_)):e.key==="b"&&u.setBinaural(!u.spatial)});let F=!1;function K(){u.running||u.start().catch(e=>console.warn("audio unavailable:",e))}function Ct(){F||(F=!0,bt.classList.add("gone"),u.open(),K())}window.addEventListener("click",Ct);let l=null;function J(e,n){P(f.x+e,f.y+n)}function Q(){l!==null&&A.hasPointerCapture(l.id)&&A.releasePointerCapture(l.id),l=null,document.body.classList.remove("dragging")}window.addEventListener("pointerdown",e=>{!e.isPrimary||e.button!==0||F&&(l={id:e.pointerId,x:e.clientX,y:e.clientY},A.setPointerCapture(e.pointerId),document.body.classList.add("dragging"),e.preventDefault())});window.addEventListener("pointermove",e=>{if(l===null||e.pointerId!==l.id)return;const n=e.clientX-l.x,t=e.clientY-l.y;l.x=e.clientX,l.y=e.clientY,J(-n,-t),e.preventDefault()});window.addEventListener("pointerup",e=>{K(),!(l===null||e.pointerId!==l.id)&&Q()});window.addEventListener("pointercancel",e=>{l!==null&&e.pointerId===l.id&&Q()});const Dt=16;function U(e,n,t){return n===WheelEvent.DOM_DELTA_PAGE?e*t:n===WheelEvent.DOM_DELTA_LINE?e*Dt:e}window.addEventListener("wheel",e=>{J(U(e.deltaX,e.deltaMode,v),U(e.deltaY,e.deltaMode,b)),e.preventDefault()},{passive:!1});window.addEventListener("resize",Y);Y();requestAnimationFrame($);
