var nt=Object.defineProperty;var st=(n,e,t)=>e in n?nt(n,e,{enumerable:!0,configurable:!0,writable:!0,value:t}):n[e]=t;var w=(n,e,t)=>st(n,typeof e!="symbol"?e+"":e,t);import{R as at,P as it}from"./sim-B88PxYSu.js";const z=4,ot=String.raw`
...
`,rt=10.2,ht=.004,lt=.28,q=6.1,ct=343,G=1.6/q;function dt(n,e){const t=G*e;return t/Math.hypot(n,t)}const ut=.62,ft=1500,pt=.9,wt=.084,gt=[1,9/8,5/4,3/2,5/3],mt=196;function yt(n){let e=n,t=1/0;for(let s=-1;s<=4;s++){const o=mt*Math.pow(2,s);for(const a of gt){const i=o*a,r=Math.abs(Math.log2(i/n));r<t&&(t=r,e=i)}}return e}function vt(n,e,t){const s=Math.floor(n.sampleRate*e),o=n.createBuffer(2,s,n.sampleRate);for(let a=0;a<2;a++){const i=o.getChannelData(a);for(let h=0;h<s;h++){const c=h/s;i[h]=(Math.random()*2-1)*Math.pow(1-c,t)}const r=Math.floor(n.sampleRate*.012);i.copyWithin(r,0,s-r),i.fill(0,0,r)}return o}class bt{constructor(){w(this,"ctx",null);w(this,"node",null);w(this,"master",null);w(this,"ready",!1);w(this,"opened",!1);w(this,"budget",0);w(this,"binaural",!0)}get running(){return this.ready}get spatial(){return this.binaural}async start(){if(this.ctx){await this.ctx.resume(),this.opened&&this.fadeIn();return}const e=window.AudioContext??window.webkitAudioContext;if(!e)throw new Error("Web Audio is unavailable");const t=new e({latencyHint:"interactive"});this.ctx=t;const s=t.resume();t.addEventListener("statechange",()=>{this.opened&&this.fadeIn()});try{const o=URL.createObjectURL(new Blob([ot],{type:"text/javascript"}));try{await t.audioWorklet.addModule(o)}finally{URL.revokeObjectURL(o)}const a=new AudioWorkletNode(t,"bowl-processor",{outputChannelCount:[z]});this.node=a;const i=t.createChannelSplitter(z);a.connect(i);const r=et=>{const I=t.createBiquadFilter();I.type="highpass",I.frequency.value=90;const E=t.createBiquadFilter();E.type="highshelf",E.frequency.value=3200,E.gain.value=-9;const x=t.createBiquadFilter();return x.type="lowshelf",x.frequency.value=320,x.gain.value=2,et.connect(I),I.connect(E),E.connect(x),x},h=t.createGain();h.gain.value=.9;const c=t.createGain();c.gain.value=1;const p=t.createConvolver();p.buffer=vt(t,1.3,3.6);const d=t.createChannelMerger(2);i.connect(d,0,0),i.connect(d,1,1),r(d).connect(h);const R=t.createChannelMerger(2);i.connect(R,2,0),i.connect(R,3,1),r(R).connect(p);const y=t.createDynamicsCompressor();y.threshold.value=-4,y.knee.value=3,y.ratio.value=12,y.attack.value=.004,y.release.value=.18;const A=t.createGain();A.gain.value=0,this.master=A,p.connect(c),h.connect(A),c.connect(A),A.connect(y),y.connect(t.destination),await s,this.opened&&this.fadeIn()}catch(o){throw await t.close().catch(()=>{}),this.ctx=null,this.node=null,this.master=null,o}}open(){this.opened=!0,this.fadeIn()}fadeIn(){const e=this.ctx,t=this.master;!e||!t||this.ready||e.state!=="running"||(t.gain.setValueAtTime(0,e.currentTime),t.gain.linearRampToValueAtTime(.85,e.currentTime+2.4),this.ready=!0)}setBinaural(e){var t;this.binaural=e,(t=this.node)==null||t.port.postMessage({type:"binaural",value:e})}beginFrame(){this.budget=10}play(e){if(!this.ready||!this.node||this.budget<=0)return!1;this.budget--;const t=e.strike,s=e.wall,o=yt(rt/Math.max(ht,e.relSize)),a=Math.min(1,e.relDv/lt),i=Math.hypot(e.dx,e.dy),r=Math.atan2(e.dx,-e.dy),h=Math.hypot(i,G*e.radius),c=h/e.radius,p=i/h,d=Math.max(0,-Math.cos(r))*p;return this.node.port.postMessage({type:"hit",f0:o,az:r,spatial:p,flight:c*q/ct,cutoff:Math.max(ft,2e4*Math.exp(-(ut*c+pt*d))),send:wt/e.presence*(1+.25*d),amp:(s?.11:.34)*a*e.presence,decay:(.5+32*e.relSize)*(s?.3:1)*(.85+.3*t.bowl.timbre)*(.7+.45*a),bright:s?-.4:-.12+a*.3,stretch:(s?.02:.004)+t.bowl.timbre*.01,contact:s?.012:.006}),!0}}const M=document.getElementById("stage"),Mt=document.getElementById("veil"),Z=document.getElementById("hud"),k=new at(M),u=new bt;function At(n){let e=n>>>0;return()=>{e=e+1831565813>>>0;let t=Math.imul(e^e>>>15,1|e);return t=t+Math.imul(t^t>>>7,61|t)^t,((t^t>>>14)>>>0)/4294967296}}const X=3,Et=[162,198,144],xt=.8/3,It=2/3,Ot=2.6,St=.21,N={x:-.644,y:-.271},_t=.08;function kt(n,e){return Math.max(n,e)/(2*It)}let g=[],v=0,b=0;const f={x:0,y:0},m={left:0,top:0,width:0,height:0};function j(n,e){const t=kt(n,e),s=Ot*t/Math.sqrt(3);return Array.from({length:X},(o,a)=>{const i=St+a*2*Math.PI/X;return{cx:Math.cos(i)*s,cy:Math.sin(i)*s,r:t}})}function Y(n){const e=n[0].r*(1+_t),t=Math.min(...n.map(o=>o.cx-e)),s=Math.min(...n.map(o=>o.cy-e));m.left=t,m.top=s,m.width=Math.max(...n.map(o=>o.cx+e))-t,m.height=Math.max(...n.map(o=>o.cy+e))-s}function V(n,e,t,s){return t<=s?e+t/2:Math.min(Math.max(n,e+s/2),e+t-s/2)}function F(n,e){f.x=V(n,m.left,m.width,v),f.y=V(e,m.top,m.height,b)}function Rt(){const n=j(v,b);Y(n);const e=At(20932);g=n.map((t,s)=>{const o=1.5+s*.35,a=.085+s*.01,i=a*xt,r=new it(t.cx,t.cy,t.r,{scale:o,drift:i*o,strength:i,jet:a,axis:(s%2===0?1:-1)*(.7+s*.9),width:.34,confine:.05,seed:17.3*(s+1)},e);return r.populate(Et[s]),r}),F(N.x*n[0].r,N.y*n[0].r),k.invalidate()}function Ct(n){const e=j(v,b),t=e[0].r/g[0].radius,s=n.x*t,o=n.y*t;Y(e),g.forEach((a,i)=>{const r=e[i];a.cx=r.cx,a.cy=r.cy,a.radius=r.r;for(const h of a.bowls)h.x*=t,h.y*=t,h.r*=t,h.m*=t*t,h.vx*=t,h.vy*=t;a.ripples.length=0}),F(s,o),k.invalidate()}function $(){const n=Math.min(window.devicePixelRatio||1,2),e=document.documentElement.clientWidth,t=document.documentElement.clientHeight;if(e<=0||t<=0)return;const s=g.length?{x:f.x,y:f.y}:null;v=e,b=t,M.width=Math.round(v*n),M.height=Math.round(b*n),k.setDpr(n),s===null?Rt():Ct(s)}const C=1/120,W=6,O=[];let S=0,B=performance.now(),D=60,L=0,T=0,_=!1;function K(n){requestAnimationFrame(K);let e=(n-B)/1e3;B=n,e>.25&&(e=.25),D+=(1/Math.max(e,1e-4)-D)*.05,S+=e;let t=0;for(;S>=C&&t<W;){for(const a of g)a.step(C);S-=C,t++}t===W&&(S=0),u.beginFrame();let s=0,o=0;O.length=0;for(const a of g){s+=a.contacts.length;for(const i of a.contacts){const r=i.x-f.x,h=i.y-f.y,c=dt(Math.hypot(r,h),a.radius),p=i.kind==="wall";for(const d of i.strikes)O.push({strike:d,wall:p,relSize:d.bowl.r/a.radius,relDv:d.dv/a.radius,dx:r,dy:h,radius:a.radius,presence:c})}a.contacts.length=0}O.sort((a,i)=>i.relDv*i.presence-a.relDv*a.presence);for(const a of O)u.play(a)&&o++;if(L+=(s/Math.max(e,1e-4)-L)*.03,T+=(o/Math.max(e,1e-4)-T)*.03,k.frame(v,b,g,f),_){const a=g.reduce((i,r)=>i+r.bowls.length,0);Z.textContent=`${D.toFixed(0)} fps   ${a} bowls   ${L.toFixed(1)} contacts/s   ${T.toFixed(1)} voices/s   audio ${u.running?"on":"off"}   ${u.spatial?"binaural":"stereo"}`}}window.addEventListener("keydown",n=>{n.key==="h"?(_=!_,Z.classList.toggle("on",_)):n.key==="b"&&u.setBinaural(!u.spatial)});let U=!1;function P(){u.running||u.start().catch(n=>console.warn("audio unavailable:",n))}function J(){U||(U=!0,Mt.classList.add("gone"),u.open(),P())}const Dt=6;let l=null;function Q(n,e){F(f.x+n,f.y+e)}function tt(){l!==null&&M.hasPointerCapture(l.id)&&M.releasePointerCapture(l.id),l=null,document.body.classList.remove("dragging")}window.addEventListener("pointerdown",n=>{!n.isPrimary||n.button!==0||(P(),l={id:n.pointerId,x:n.clientX,y:n.clientY,travel:0},M.setPointerCapture(n.pointerId),document.body.classList.add("dragging"),n.preventDefault())});window.addEventListener("pointermove",n=>{if(l===null||n.pointerId!==l.id)return;const e=n.clientX-l.x,t=n.clientY-l.y;l.x=n.clientX,l.y=n.clientY,l.travel+=Math.abs(e)+Math.abs(t),l.travel>Dt&&J(),Q(-e,-t),n.preventDefault()});window.addEventListener("pointerup",n=>{P(),!(l===null||n.pointerId!==l.id)&&(J(),tt())});window.addEventListener("pointercancel",n=>{l!==null&&n.pointerId===l.id&&tt()});const Lt=16;function H(n,e,t){return e===WheelEvent.DOM_DELTA_PAGE?n*t:e===WheelEvent.DOM_DELTA_LINE?n*Lt:n}window.addEventListener("wheel",n=>{Q(H(n.deltaX,n.deltaMode,v),H(n.deltaY,n.deltaMode,b)),n.preventDefault()},{passive:!1});window.addEventListener("resize",$);$();requestAnimationFrame(K);
